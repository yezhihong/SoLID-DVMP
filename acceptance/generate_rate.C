...
int main(){
	...
	TH2F *pimf_p_theta=new TH2F("pimf_p_theta",Form("P_{%s} vs. #theta_{%s}  (forward angle)",particle.Data(),particle.Data()),Nbin_theta,6.0,26.0,Nbin_p,0,10.);
	//forward angle
	pimf_p_theta->GetYaxis()->SetTitle("P_{#pi-}");
	pimf_p_theta->GetXaxis()->SetTitle("#theta_{#pi-}");
	//large angle
	TH2F *piml_p_theta=new TH2F("piml_p_theta",Form("P_{%s} vs. #theta_{%s}  (large angle)",particle.Data(),particle.Data()),Nbin_theta,6.0,26.0,Nbin_p,0,10.);
	piml_p_theta->GetYaxis()->SetTitle("P_{#pi-}");
	piml_p_theta->GetXaxis()->SetTitle("#theta_{#pi-}");
    //total
	TH2F *pim_p_theta=new TH2F("pim_p_theta",Form("P_{%s} vs. #theta_{%s}  (large+forward angle)",particle.Data(),particle.Data()),Nbin_theta,6.0,26.0,Nbin_p,0,10.);
	pim_p_theta->GetYaxis()->SetTitle("P_{#pi-}");
	pim_p_theta->GetXaxis()->SetTitle("#theta_{#pi-}");
    /*}}}*/
    
	/*_____TH1F of theta for pi-{{{*/
	//forward angle
	TH1F *pimf_theta=new TH1F("pimf_theta",Form("rate as function of #theta_{%s}  (forward angle)",particle.Data()),Nbin_theta,6.0,26.0);
	pimf_theta->GetYaxis()->SetTitle("rate (Hz)");
	pimf_theta->GetXaxis()->SetTitle("#theta_{pi-}");
	//large angle
	TH1F *piml_theta=new TH1F("piml_theta",Form("rate as function of #theta_{%s}  (large angle)",particle.Data()),Nbin_theta,6.0,26.0);
	piml_theta->GetYaxis()->SetTitle("rate (Hz)");
	piml_theta->GetXaxis()->SetTitle("#theta_{pi-}");
	//total
	TH1F *pim_theta=new TH1F("pim_theta",Form("rate as function of #theta_{%s}  (large+forward angle)",particle.Data()),Nbin_theta,6.0,26.0);
	pim_theta->GetYaxis()->SetTitle("rate (Hz)");
	pim_theta->GetXaxis()->SetTitle("#theta_{pi-}");
    /*}}}*/

	/*_____TH1F of p for pi-{{{*/
	//forward angle
	TH1F *pimf_p=new TH1F("pimf_p",Form("rate as function of P_{%s}  (forward angle)",particle.Data()),Nbin_p,0.0,10.);
	pimf_p->GetYaxis()->SetTitle("rate (Hz)");
	pimf_p->GetXaxis()->SetTitle("P_{N}");
	//large angle
	TH1F *piml_p=new TH1F("piml_p",Form("rate as function of P_{%s}  (forward angle)",particle.Data()),Nbin_p,0.0,10.);
	piml_p->GetYaxis()->SetTitle("rate (Hz)");
	piml_p->GetXaxis()->SetTitle("P_{N}");
	//total
	TH1F *pim_p=new TH1F("pim_p",Form("rate as function of P_{%s}  (large+forward angle)",particle.Data()),Nbin_p,0.0,10.);
	pim_p->GetYaxis()->SetTitle("rate (Hz)");
	pim_p->GetXaxis()->SetTitle("P_{N}");
	/*}}}*/
    
	/*}}}*/
	
    /*proton{{{*/
	/*_____p vs theta for p{{{*/
	TH2F *pf_p_theta=new TH2F("pf_p_theta",Form("P_{%s} vs. #theta_{%s}  (forward angle)",particle.Data(),particle.Data()),Nbin_theta,0.0,70.0,Nbin_p,0,1.5);
	//forward angle
	pf_p_theta->GetYaxis()->SetTitle("P_{p}");
	pf_p_theta->GetXaxis()->SetTitle("#theta_{p}");
	//large angle
	TH2F *pl_p_theta=new TH2F("pl_p_theta",Form("P_{%s} vs. #theta_{%s}  (large angle)",particle.Data(),particle.Data()),Nbin_theta,0.0,70.0,Nbin_p,0,1.5);
	pl_p_theta->GetYaxis()->SetTitle("P_{p}");
	pl_p_theta->GetXaxis()->SetTitle("#theta_{p}");
    //total
	TH2F *p_p_theta=new TH2F("p_p_theta",Form("P_{%s} vs. #theta_{%s}  (large+forward angle)",particle.Data(),particle.Data()),Nbin_theta,0.0,70.0,Nbin_p,0,1.5);
	p_p_theta->GetYaxis()->SetTitle("P_{p}");
	p_p_theta->GetXaxis()->SetTitle("#theta_{p}");
    /*}}}*/
    
	/*_____TH1F of theta for p{{{*/
	//forward angle
	...
	pf_theta->GetXaxis()->SetTitle("#theta_{p}");
	//large angle
	...
	pl_theta->GetXaxis()->SetTitle("#theta_{p}");
	//total
	...
	p_theta->GetXaxis()->SetTitle("#theta_{p}");
    /*}}}*/

	/*_____TH1F of p for p{{{*/
	//forward angle
	TH1F *pf_p=new TH1F("pf_p",Form("rate as function of P_{%s}  (forward angle)",particle.Data()),Nbin_p,0.0,1.5);
	pf_p->GetYaxis()->SetTitle("rate (Hz)");
	pf_p->GetXaxis()->SetTitle("P_{N}");
	//large angle
	TH1F *pl_p=new TH1F("pl_p",Form("rate as function of P_{%s}  (forward angle)",particle.Data()),Nbin_p,0.0,1.5);
	pl_p->GetYaxis()->SetTitle("rate (Hz)");
	pl_p->GetXaxis()->SetTitle("P_{N}");
	//total
	TH1F *p_p=new TH1F("p_p",Form("rate as function of P_{%s}  (large+forward angle)",particle.Data()),Nbin_p,0.0,1.5);
    ...
    for(Long64_t i=0;i<N_entries;i++){
        ...
        if(ScatElec_Theta_Col>7.5&&ScatElec_Theta_Col<24.5&&ScatElec_Mom_Col>1.&&ScatElec_Mom_Col<11
          ...
          &&Qsq>1.0&&ZASigmaPara>1e-33){//any additional cuts should be added in here
    ...
    /*        cerr<<Form("--- eP=%f, eTheta=%f, pP=%f, pTheta=%f", ScatElec_Mom_Col,ScatElec_Theta_Col, Pion_Mom_Col,Pion_Theta_Col)<<endl;*/
           ...
           /*}}}*/
            ...
            /*Pions{{{*/
			//p vs theta g
			pimf_p_theta->Fill(Pion_Theta_Col,Pion_Mom_Col, event_weight*forward_acceptance );
			piml_p_theta->Fill(Pion_Theta_Col,Pion_Mom_Col, event_weight*large_acceptance );
			pim_p_theta->Fill( Pion_Theta_Col,Pion_Mom_Col, event_weight*total_acceptance );
			//TH1F theta
			pimf_theta->Fill(Pion_Theta_Col,event_weight*forward_acceptance );
			piml_theta->Fill(Pion_Theta_Col,event_weight*large_acceptance );
			pim_theta->Fill( Pion_Theta_Col,event_weight*total_acceptance );
			//TH1F p
			pimf_p->Fill(Pion_Mom_Col,event_weight*forward_acceptance );
			piml_p->Fill(Pion_Mom_Col,event_weight*large_acceptance );
			pim_p->Fill( Pion_Mom_Col,event_weight*total_acceptance );
            /*}}}*/
            
            /*Protons{{{*/
			//p vs theta g
			pf_p_theta->Fill(Proton_Theta_Col,Proton_Mom_Col, event_weight );
			pl_p_theta->Fill(Proton_Theta_Col,Proton_Mom_Col, event_weight);
			p_p_theta->Fill( Proton_Theta_Col,Proton_Mom_Col, event_weight );
			//TH1F theta
			pf_theta->Fill(Proton_Theta_Col,event_weight);
			pl_theta->Fill(Proton_Theta_Col,event_weight);
			p_theta->Fill( Proton_Theta_Col,event_weight);
			//TH1F p
			pf_p->Fill(Proton_Mom_Col,event_weight);
			pl_p->Fill(Proton_Mom_Col,event_weight);
			p_p->Fill( Proton_Mom_Col,event_weight);
            /*}}}*/
